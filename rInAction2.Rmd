...
|    位置    |   面板选项    |          描述          |
| :--------: | :-----------: | :--------------------: |
| 非主对角线 |   panel.pie   |   饼图表示相关性大小   |
|            |  panel.shade  |   阴影表示相关性大小   |
|            | panel.ellipse |      绘制置信椭圆      |
|            |   panel.pts   | 平滑拟合曲线绘制散点图 |
|  主对角线  | panel.minmax  |   输出变量最大最小值   |
|            |   panel.txt   |      输出变量名字      |

### 马赛克图

对于多个类别型变量,其中嵌套矩形面积正比于单元格频率,该频率即是多维列联表的频率,颜色或阴影表示拟合模型残差值
有自带的mosaicplot(),但推荐使用使用vcd包中的mosaic()
对泰坦尼克数据集
```{r}
ftable(Titanic)
```
...
```{r}
library(boot)
rsp <- function(formula, data, indices){
  d <- data[indices,]
  fit<-lm(formula,data=d)
  return(summary(fit)$r.square)
}
results<-boot(data=mtcars, statistic=rsp, R=1000, formula=mpg~wt+disp)
results
plot(results)
```
